...
export function ChefCard({ chef }: ChefCardProps) {
  return (
    <Link href={`/chefs/${chef.slug}`} className="group block">
      <article className="rounded-2xl border border-neutral-100 bg-white overflow-hidden transition-shadow hover:shadow-lg">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-neutral-400">{chef.location}</p>
              </div>
            </div>
            <span className="text-xs text-neutral-300 whitespace-nowrap">{chef.priceRange === 1 ? "$" : chef.priceRange === 2 ? "$$" : "$$$"}</span>
          </div>
          <p className="mt-3 text-sm text-neutral-500 line-clamp-2">{chef.title}</p>
